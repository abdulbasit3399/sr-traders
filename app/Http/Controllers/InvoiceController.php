...
class InvoiceController extends Controller
{
    ...
    public function returnstore(Request $request)
    {
        ...
        if (\Auth::user()->can('create invoice')) {
            ...
            $invoice->status         = 0;
            $invoice->return_date    = $request->return_date;
            ...
            return redirect()->route('invoice.returnIndex', $invoice->id)->with('success', __('Invoice successfully created.'));
        } else {
            return redirect()->back()->with('error', __('Permission denied.'));
        }
    }
    ...
    // {
    //     return view('InvoiceReturn.index');
    //     if (\Auth::user()->can('manage customer invoice')) {
    ...
    //         return view('InvoiceReturn.index', compact('invoices', 'status'));
    //     } else {
    //         return redirect()->back()->with('error', __('Permission Denied.'));
    //     }
    // }
